...
class TestCommentMethods:
    def test_add_comment(self, in_memory_repository, test_user,test_post, test_comment):
    ...
    def test_add_multiple_comments(self, in_memory_repository, test_user, test_post, test_comment):
        comment2 = Comment(999,test_user.id, test_post.id, datetime.now(), "Another test comment", 0)
        in_memory_repository.add_multiple_comments([test_user, test_user], [test_comment, comment2])
        comments = in_memory_repository.get_comments_by_post(test_post)
        assert test_comment in comments
        assert comment2 in comments

    def test_add_like_to_comment(self, in_memory_repository, test_user, test_post, test_comment):
        in_memory_repository.add_comment(test_user, test_comment)
    ...
    def test_delete_comment(self, in_memory_repository, test_user, test_post, test_comment):
        in_memory_repository.add_comment(test_user, test_comment)
